...
>
> **版本**：**v3**（2026-09-15）— 变更沿革：
> - **v2**：① 新增「**三·补、RAW 转售边界（合规界定）**」 ② §五 修正为**两档**
> - **v3**：③ 消除「不做 SaaS」口径矛盾（与 §一"SaaS 也留"对齐） ④ 修订过时数字（validation 场景 / 输出形态 / LOC / 测试文件） ⑤ §二 市场数据补真实来源、无法核实项明确标注 ⑥ §三·补 补 default-deny 的直接后果
>
> ⚠️ 若你手上这份**没有「三·补」或版本不是 v3** → 请以最新版为准。
...
| 事实 | 数据 | 出处 | 含义 |
|:--|:--|:--|:--|
| 通用 AI 研究=红海 | 大厂 deep research 均已内置到 $20/月档 | **【未验证】** | ❌ 不做"通用信息入口" |
| 横向层已出清 | ⚠️ **原稿数字不可用**：原写"5,600+ 初创关门 / 78% 是 wrapper"**查不到可靠来源**。可查口径为：2025 年 AI 公司占全部初创倒闭的 **16%**，Series A 倒闭占比同比 6%→**14%** | SimpleClosure 2025 年数据（经媒体转述） | ⚠️ **方向不变**（通用层拥挤、出清加快），但**原数字不得再引用** |
| **成功案例全靠独特资产** | AlphaSense **$700M ARR** | Sacra；The Information 报道 | ✅ **卖结果/数据才是出路** |
| | Glean **$100M ARR** | Glean 官方新闻稿（2025-02） | |
| | Recorded Future **$2.65B 被 Mastercard 收购** | Mastercard 官方新闻稿（2024-09） | |
| 垂直情报能收高溢价 | AlphaSense $10-20k/席/年 | **【未验证】** | ✅ 与"卖结果"模式一致 |
| **稀缺能力** | 跨源去重/实体消解、持续追踪、真正可验证性 | 内部判断（非外部数据） | ✅ **这三条是我们的机会** |
| 我们的核心能力在大厂优势区 | 聚合 + 结构化 | 内部判断（非外部数据） | ⚠️ 必须靠"可验证 + 持续"差异化 |

> **使用纪律**：标 **【未验证】** 的项对外沟通前须补来源或替换；标"原稿数字不可用"的**不得再引用**。
