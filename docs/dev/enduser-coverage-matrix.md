...
> Code 76% (75/99) | Validation 76% (75/99) | 双向 60% (59/99) | 可覆盖上限 93/99（排除 6 项纯无解）
...
| A18 | **付费新闻/通讯社** | WSJ, FT, 财新, 新华社, 人民日报 | ❌ 无 API | ❌ 未测试 | ❌ |
...
| A23 | **社科/法律工作论文** | SSRN | ❌ 无 collector（SSRN 有限 API/RSS 未接入） | ❌ 未测试 | ❌ |
| A24 | **开源数据集** | Hugging Face, Kaggle | ❌ 无 collector | ❌ 未测试 | ❌ |
| A25 | **学术付费数据库** | Elsevier/Scopus, Springer Nature, IEEE Xplore | ❌ 机构付费许可 | ❌ 未测试 | ❌ |
...
|------|:----:|
| 报告推荐源总数 | 29 |
| AutoInfo Code 已覆盖 | 19/29 (66%) |
| Validation Plan 已测试 | 19/29 (66%) |
| 双向覆盖（Code + Plan） | 17/29 (59%) |
| 完全未覆盖 | 10/29 (34%) |
...
| C11 | **播客平台目录** | #9 | B 被动推送 | ⚠️ B11 生成 MP3 音频，但未发布到 Spotify/Apple 播客目录 | ⚠️ B11 生成已验证（Part 4 Q36e），目录发布未验证 | ⚠️ |
...
| E11 | **API 数据许可/RAW 产品** | 报告 §8.3：API/数据许可（Reddit-Google $60M/年） | ⚠️ ProductType.RAW（每域 raw feed + api 交付）；webhook 流/批量导出非独立变体 | ⚠️ 未深度测试 | ⚠️ |
| E12 | **单篇/Micro-subscription** | 报告 §8.3（Substack IAP、单篇 $0.25-$15） | ❌ 无单篇购买（Stripe 订阅已有，可扩展） | ❌ 未测试 | ❌ |
| E13 | **RaaS 效果付费** | 报告 §8.3、§7.3 价值透明化 | ❌ 无按效果计费（E3 用量计量是基础） | ❌ 未测试 | ❌ |
| E14 | **内容简化** | 报告 §6.5：30% 场景 | ❌ 未实现（CEFR 语言分级相邻，非通用简化） | ❌ 未测试 | ❌ |
...
|:----|:---------:|:---------:|:------:|:---------:|:------:|:--------:|:------:|
| **A. 原始资讯源** | 29 | 18 | **62%** | 18 | **62%** | 16 | **55%** |
| **B. 输出产品** | 25 | 21 | **84%** | 21 | **84%** | 19 | **76%** |
| **C. 分发渠道** | 14 | 10 | **71%** | 10 | **71%** | 8 | **57%** |
| **D. 领域覆盖** | 16 | 15 | **94%** | 15 | **94%** | 9 | **56%** |
| **E. Agent 触达** | 15 | 11 | **73%** | 11 | **73%** | 7 | **47%** |
| **总计** | **99** | **75** | **76%** | **75** | **76%** | **59** | **60%** |
...
|:--:|------|---------|
| B15 | PDF 导出 | 超时未深度测试 |
| E2 | Stripe 支付生命周期 | 需真实 `STRIPE_API_KEY`（Part 13 可降级演练） |
...
> 报告明确列出的可接入平台/可扩展产品，尚未实现：

| 项 | 功能 | 报告依据 | 实现路径 |
|:--:|------|---------|---------|
| A23 | SSRN 社科工作论文 | §4.1 有限 API/RSS 大部分免费 | RSS 接入（同 Substack 模式） |
| A24 | Hugging Face / Kaggle 数据集 | §3.1 数据/AI 训练格式 | HF datasets-server API |
| A28 | TikTok | §4.5 Research API | 需学术审核（Research API 准入） |
| B23 | 电子书/音频书输出 | §3.1 教育格式 3 | epub/mobi 导出扩展 |
| E12 | 单篇/Micro-subscription | §8.3 Substack IAP 模式 | 基于现有 Stripe checkout 扩展 |
| E14 | 内容简化 | §6.5 30% 场景 | LLM simplify 输出模式 |
...
| A18 | WSJ / FT / 财新 / 新华社 / 人民日报 | 付费墙或无开放 API |
| A19 | 知乎 / 得到 / 微信公众号 | 无公开 API，反爬严格 |
| A20 | X / 微博 / 抖音 / 小红书 | 付费 API 或封锁抓取 |
| A25 | Elsevier / Springer / IEEE | 机构付费许可 |
...
1. **全量对齐后代码覆盖率 76%**（75/99）— 新增 33 项报告场景（多为不可工程化 ❌ 或部分覆盖 ⚠️），覆盖率从 94% 回落是如实反映报告全部场景，而非功能回退
2. **验证覆盖率 76%**（75/99），**双向覆盖 60%**（59/99）
3. **新增缺口全部为"合理未覆盖"**：P3 不可工程化 8 项（机构付费墙/无 API；A29 中文播客已于 2026-08-02 实测确认 Apple Podcasts 隐式覆盖，移出 P3）+ P4 范围外 8 项，结构性无法或不应覆盖
4. **可工程化但未实现 7 项**（P2：SSRN / HF-Kaggle / TikTok / 电子书 / 单篇订阅 / 内容简化 / A2A）— 下一步开发的优先清单
5. **报告 §6.5 七大 AI 使用场景已映射**：追问/获取新闻/摘要/跨源/翻译 5 项已覆盖（E5/B8/B20），可信度评估部分覆盖（E9=G1 分级），内容简化未实现（E14）
...
| 类别 | 项 | 原因 | 实现路径 |
|:----:|:--:|------|---------|
| Code 缺失 | A23 SSRN | 有限 API/RSS 未接入 | RSS 接入（同 Substack 模式） |
| Code 缺失 | A24 Hugging Face/Kaggle | 无 collector | HF datasets-server API |
| Code 缺失 | A28 TikTok | Research API 需学术审核 | 准入后接入 |
| Code 缺失 | B23 电子书/音频书 | 无 epub 输出 | epub/mobi 导出扩展 |
| Code 缺失 | E12 单篇/Micro 订阅 | 无单篇购买 | Stripe checkout 扩展 |
| Code 缺失 | E14 内容简化 | 无 simplify 输出 | LLM 输出模式 |
| Code 缺失 | E15 A2A 原生协议 | webhook 单向 | A2A server 实现 |

### ② 合理未覆盖 — 外部限制（8 项）— 机构付费墙或无公开 API，结构性无法覆盖

| 类别 | 项 | 原因 |
|:----:|:--:|------|
| 代码缺失 | A7 Bloomberg/Refinitiv/Wind/Choice/iFinD/CEIC | 机构级付费 API，无公开接口 |
| 代码缺失 | A18 WSJ/FT/财新/新华社/人民日报 | 付费墙或无开放 API |
| 代码缺失 | A19 知乎/得到/微信公众号 | 无公开 API，反爬严格 |
| 代码缺失 | A20 X/微博/抖音/小红书 | 付费 API（X 涨价）或封锁抓取 |
| 代码缺失 | A25 Elsevier/Springer/IEEE | 机构付费许可 |
...
### H1. 生产实现清单（V1 — 全部免费零成本）

| 项 | 功能 | 实现路径 | 核实依据 | 成本 |
|:--:|---|---|:---:|:---:|
| **A23** | SSRN 社科工作论文 | RSS 接入（同 Substack 模式） | 有限 API/RSS 大部分免费 | 低 0.5-1d |
| **A24** | Hugging Face / Kaggle | HF datasets-server 公开 API + Kaggle API | 公开免费 API | 中 2-3d |
| **A25** | 学术付费库 OA 全文 | Unpaywall（10 万次/天）+ CORE 免费 OA 全文（元数据 OpenAlex 已有） | ✅ 核实免费可用 | 中 2-3d |
| **A18** | 新闻头条级覆盖 | GDELT 免费（无 key、3 个月窗口）+ Google News RSS；高价值内容走机构授权（付费可选，独立决策） | ✅ 核实免费可用 | 低-中 1-2d |
...
| **E12** | 单篇/Micro 订阅 | Stripe `mode="payment"`（与订阅共存） | ✅ 核实 | 低 1-2d |
| **E14** | 内容简化 | LLM simplify 输出模式（新增 output mode） | — | 低 0.5-1d |
| **E11** | RAW 产品三变体 | 拆分 api feed / webhook 流 / 批量导出（README 已声明三变体，代码仅单一 raw feed） | 文档-代码一致性 | 中 1-2d |
| **E9** | 来源可信度评分 | G1 分级已有 + LLM 可信度评分扩展 | — | 低 0.5-1d |
| **C11** | 播客目录发布 | B11 音频已有 + 标准播客 RSS 发布 | — | 低-中 1-2d |

### H2. 验证补齐清单（V1 — 免费测试凭证即可）
...
| E15 | A2A 原生协议 | 代码实现（MCP 138 工具已覆盖 Agent 对接） | 生态未成熟 |
