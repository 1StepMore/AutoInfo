...
| **D3 Validation 与 agent-as-tester** | 验证体系本身符合业界吗:LLM-judge 校准、scenario 设计、agent-as-tester 先例、CI 质量门 | `acceptance-framework`、`validation-scenario-contract`、116 场景、2 次运行报告 |
...
| 错误四问模型:人类可读 message + 机器可读 code + 4xx/5xx 分层 + 可操作下一步;不泄漏堆栈/内部 ID | OWASP(信息泄漏=A10:2025);Reclear;AgentPlaybook | `{success,error:{code,message,actionable}}` + 26 ErrorCode + `LLM_NOT_CONFIGURED` 带提示 — 基本实现 | 🟡 部分(需核对:错误码是否 snake_case 可机器处理?429 是否带 Retry-After?堆栈是否可能泄漏到 REST 响应?) |
...
| 可操作错误信息:错误码 + 人类/LLM 可读消息 + 建议修复;不返回裸 HTTP 状态码 | Anthropic;AWS | envelope 已实现;26 ErrorCode message 是否都含"具体修复"需核对 | 🟡 部分 |
...
| 业界主张 | 来源 | AutoInfo 对照 | 结论 |
|---------|------|--------------|------|
| scenario/task-based agent 验证是 2024-2026 标准形态:Anthropic evals、τ-bench(数据库终态比对 + pass^k)、AutoCover(Uber,5-agent + mutation 质量门)、SpecOps(4-agent,F1=0.89)、TestExplora(主动找 bug F2P 仅 16%) | Anthropic;τ-bench;ICSE 2026 | 116 场景 + agent-tester 执行 + LLM-judge + 验收报告 — **与业界前沿同形态** | ✅ 已满足(强项,可引为行业先例) |
...
| capability evals(从低 pass 测进步)vs regression evals(~100% 防回退);饱和后晋升 | Anthropic | 116 场景已分 functional + regression ✅;capability/regression 双套件显式化 + 饱和轮换机制未文档化 | 🟡 部分 |
...
| D-工-4 | 错误信息一致性审计(26 ErrorCode 的 message 是否全含修复指引;429 Retry-After;堆栈泄漏) | OWASP;MCP spec | 低-中 | 高 | ✅ **已修复** → `scripts/error_message_audit.py`(2026-08-14 落地,2026-08-23 重跑,0 缺修复指引,**0 裸异常**——65 处 `_error_dict(exc)` 全部替换为 `_error_from_exc(exc, context)` 统一模板(context + 异常 + retry 指引),回归测试 9 例) |
...
1. **agent-as-tester validation 体系** — 与 Anthropic/τ-bench/AutoCover/SpecOps 同形态;116 场景真实面 + 诚实性机制 + 回归飞轮,是行业前沿而非孤例(D3-1)。
...
2. 场景从"路径断言"转向"outcome 断言"—— 已 ≥95% 达标(2026-08-23 重跑 116 场景/446 步),残余 12 步无 success 键(`scripts/scenario_outcome_audit.py`,D3-1)。
