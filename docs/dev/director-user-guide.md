...
| **Direct User** (Agent / 直接执行者) | The AI agent that executes commands. Translates your NL into MCP tool calls, runs collection pipelines, generates output, reports results. | MCP tools (139 tools, 34 categories). CLI is fallback. | Calling `collect_sources()`, `generate_digest()`, `search_knowledge_base()` |
...
The agent has 139 MCP tools organized into 34 categories. Everything AutoInfo can do, the agent can do on your behalf:
