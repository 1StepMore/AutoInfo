...
baseline): run `list_validation_scenarios()` (expect 138) then `run_validation_scenario`
...
/ 10 unconfigured**; the suite has since grown to 138 (65 functional + 73 regression; §1.8) — the baseline shape (all
...
## 1.8 Scenario inventory (as of 2026-09-01)

138 scenario files in `src/autoinfo/mcp/scenarios/` (65 functional flat in `scenarios/`
+ 73 regression in `scenarios/regression/`):
...
- **Regression (scenarios/regression/)**: regression-collect-int-id (#104),
  ...
  #119/#120 cross-product-coherence wave additions:
  regression-cross-product-same-source (#119 — digest + presentation share ≥0.5
  source-host overlap on a fixture KB, drifted sources absent from both, via a
  seeded fixture + a synthetic temp-cwd config so the real drift-filter seam is
  ...
  is unavailable).
  2026-09-04 concierge-wave additions (full live list:
  `ls src/autoinfo/mcp/scenarios/regression/*.yaml | wc -l` — 72 at last count):
  regression-qa-gates-dir, regression-mvp-init, regression-free-tier-limit,
  regression-readonly-mcp, regression-domain-seed-flagship,
  regression-sources-sections, regression-validation-list,
  regression-product-quality-all-templates, regression-report-entries-cap,
  regression-tutorial-entry-cap.
  Each carries
...
Coverage: 146/146 MCP tools (100%), all 31 CLI command groups, 8 REST API endpoints,
...
**146 MCP tools (35 categories), 31 CLI command groups, 8 REST endpoints, 13 delivery
...
2. **No-keys profile.** Record the RED baseline (§0.4): with no BYOK keys, run
   `list_validation_scenarios()` (expect 138) then `run_validation_scenario` for every
...
1. Re-run 138 scenarios with keys configured — all must report `passed` (expect 0
...
- Scenario suite: **138/138 passed** (0 failed, 0 unconfigured) with keys set.
...
- [ ] All 138 scenarios re-run GREEN with keys configured (0 failed, 0 unconfigured); `python3 scripts/coverage_audit.py` reports 146/146 with zero MISSING.
