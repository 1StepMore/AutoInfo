<!-- doc-type: analysis -->
# End-User Service Coverage Matrix — historical analysis (archived)

> **Archived 2026-09-13 (Todo 7, agent-oriented-gap-register).** This is the
> pre-generation hand-maintained snapshot of `docs/dev/enduser-coverage-matrix.md`.
> The live 99-item coverage view is now **generated** from
> `docs/dev/specs/end-user-matrix.yaml` (`report_demand`) by
> `python3 scripts/coverage_matrix.py --render-enduser-doc`.
>
> This file preserves the historical update log, feasibility verdicts, gap
> roadmap and issue analysis verbatim. It is **not authoritative** for current
> coverage counts — do not cite its numbers; regenerate the live view instead.
...
> 2026-08-13 更新（第 13 次，video wave）：**B12 视频摘要升级为 HyperFrames 管线** — 替换 PIL+FFmpeg slideshow scaffold 为 HTML+GSAP→MP4（`src/autoinfo/output/video.py` + `video_assets/`：36 主题 + 8 品牌主题库、6 视觉布局强制相邻场景多样性、AutoMedia 场景帧边界数学）。MCP `generate_report`/`generate_cross_domain_report` `format="video"` 枚举 + persist（.mp4）+ 验证场景 `output-video`（scenarios 67→68）。13/13 demo 域真实渲染 MP4 evidence（~500KB each）。**LLM reasoning 控制**：`reasoning_model=True` 默认关闭 chain-of-thought（`additional_body={"thinking":{"type":"disabled"}}`）防 JSON 截断；判断类调用点（G4 事实/G5 翻译/llm_judge/翻译QA裁判/验证裁判）以 `disable_thinking=False` + 提高 max_tokens 保留推理。**矩阵 evidence 修复**：`coverage_matrix.py` scan_evidence 双重路径 bug 修复 + manifest/zip 扫描限域，produced 0→26（video 13/13）、source_gaps 89→38。B12 行 Code 列更新为 HyperFrames 描述。
...
> Code 90% (89/99) | Validation 90% (89/99) | 双向 87% (86/99) | 可覆盖上限 93/99（排除 6 项纯无解）| MCP 工具验证 146/146（2026-08-13 第 11 次：16 项 item 行更新，Code 83→89；D 维双向口径=有对应 Demo 域 13/16）
>
