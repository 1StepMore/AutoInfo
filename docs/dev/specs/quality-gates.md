...
The MCP server uses a unified error response system (`src/autoinfo/mcp/errors.py`) that provides consistent error classification across all 141 MCP tools. The `ErrorCode` enum (27 values) covers all known failure modes and includes seven codes added since v1.8:
