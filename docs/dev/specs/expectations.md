...
| **Error responses** | Unified dual-format (flat + envelope) for backward-compatible consumer migration. `ErrorCode` enum (28 values) covers NotFound, DomainNotFound, ValidationError, InternalError, and additional codes: `AuthRequired` (future SSE auth), `RateLimited` (future rate limiting), `SessionExpired` (future session management), `LLMNotConfigured` (LLM guard, v1.8.1), `NoCachedItems`, `EmptyResult`, `ConfigNotFound`. Flat format returns `{error_code, message, actionable}`; envelope format returns `{success: false, error: {code, message, actionable}}`. |
