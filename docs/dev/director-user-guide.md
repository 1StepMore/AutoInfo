...
| **Direct User** (Agent / 直接执行者) | The AI agent that executes commands. Translates your NL into MCP tool calls, runs collection pipelines, generates output, reports results. | MCP tools (132 tools, 32 categories). CLI is fallback. | Calling `collect_sources()`, `generate_digest()`, `search_knowledge_base()` |
