...
def _send_test_email(email_cfg: EmailConfig) -> None:
    ...
    if not email_cfg.smtp_host:
        fail_if_global("EmailNotEnabled", "SMTP server not configured. Use --smtp-server first.")
    ...
    if not from_addr:
        fail_if_global(
            "EmailNotEnabled", "No from address configured. Set email.from_addr in config."
        )
        typer.echo("Error: No from address configured. Set email.from_addr in config.", err=True)
        raise typer.Exit(1)

    to_addrs = email_cfg.to_addrs
    if not to_addrs:
        fail_if_global("EmailNotEnabled", "No recipients configured. Set email.to_addrs in config.")
        typer.echo("Error: No recipients configured. Set email.to_addrs in config.", err=True)
        raise typer.Exit(1)
    ...
    try:
        ...
        fail_if_global("EmailSendFailed", f"SMTP delivery failed: {exc}")
        typer.echo(f"Error: SMTP delivery failed: {exc}", err=True)
        raise typer.Exit(1) from exc
    except Exception as exc:
        fail_if_global("EmailSendFailed", f"Test email failed: {exc}")
...
def send_digest(
    ...
    try:
        ...
        fail_if_global("EmailSendFailed", str(exc))
...
@app.command()
def config(
    smtp_server: Optional[str] = typer.Option(None, "--smtp-server", help="SMTP server hostname"),
    smtp_port: Optional[int] = typer.Option(None, "--smtp-port", help="SMTP server port"),
    username: Optional[str] = typer.Option(None, "--username", help="SMTP username"),
    password: Optional[str] = typer.Option(None, "--password", help="SMTP password"),
    enable: bool = typer.Option(False, "--enable", help="Enable email sending"),
    disable: bool = typer.Option(False, "--disable", help="Disable email sending"),
    test: bool = typer.Option(False, "--test", help="Send a test email using current config"),
) -> None:
    ...
    if enable and disable:
        fail_if_global("ValidationError", "Cannot use both --enable and --disable.")
    ...
    if emit_if_global(
        {
            "smtp_server": email_cfg.smtp_host,
            "smtp_port": email_cfg.smtp_port,
            "username": email_cfg.smtp_user,
            "password_set": bool(email_cfg.smtp_pass),
            "from_addr": email_cfg.from_addr,
            "to_addrs": list(email_cfg.to_addrs),
