...
信息摄入 → 脱壳清洗 → 结构化 → 永久入库
 ...
 (deprecated — scaffolded but never written to)
```

**核心规则**（2026-07-01 用户确认，写进 `_kb-reform-design.md`）：

- **01-Raw 是唯一入口**，所有入库默认直接进 01-Raw（00-Inbox 仅作为目录骨架保留，无代码写操作）
...
### 00-Inbox —— 电光火石（⚠️ deprecated）

> **注意：00-Inbox 已被弃用。所有信息直接进入 01-Raw，不再经由 00-Inbox。**
>
> 以下为历史描述，仅作参考。

原设计为临时堆放快进快出的信息：安全事件、运行记录、周报草稿等。
**每日清理**——不归档就删。但实际代码从未写入此目录——`cli/init.py` 创建骨架但无任何写入路径。
...
| 层级 | 文件数 | 占比 | 说明 |
|:----|:------:|:----:|:-----|
| 00-Inbox | 8 | 0.9% | ⚠️ deprecated — 骨架目录，无代码写入 |
| 01-Raw | 698 | **77.2%** | 原始素材，**唯一入口**，知识库的"毛坯层" |
...
外部信息（无限）
    ↓
00-Inbox（⚠️ deprecated — 仅骨架，无写入）
    ↓
01-Raw（698，77%） ←——— 所有信息的**真正第一入口**
    ↓ 清洗/结构化
