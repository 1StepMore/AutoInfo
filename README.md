...
| CLI | ✅ 31 command groups (init, doctor, collect, process, status, summaries, sources, topics, topic-group, domain, audit, kb, output, cron, knowledge, cefr, email, keywords, clean, cost, billing, enduser, portal, trace, import-kb, query-collected, alert-rules, agent-callback, validation, serve, mvp) |
...
| Validation scenarios | ✅ 166 scenarios (86 functional + 80 regression in `src/autoinfo/mcp/scenarios/regression/`, `regression: true` key, recursive-glob auto-load) |
...
| Regression flywheel | ✅ `src/autoinfo/mcp/scenarios/regression/` (80 regression scenarios) + `coverage_audit.py` "Regression scenarios: N" metric + `.github/ISSUE_TEMPLATE/bug_report.md` mandatory 回归场景 field |
...
3. **Validate** — `list_validation_scenarios()` (166 scenarios: 86 functional + 80 regression) → `run_validation_scenario(scenario="system-health")`
...
autoinfo domain add|list|show|remove|activate|deactivate|import|init <name> [--seed]  # Domain management (import --from-demo supports all 21 demo domains; init --seed writes extract_fields into the seeded sources.yaml)
...
autoinfo billing summary|create-free  # Billing & usage overview; create-free provisions a free-tier user (--user-id)
